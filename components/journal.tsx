...
export function Journal() {
  ...
  const [emojiOpen, setEmojiOpen] = useState<"draft" | "reaction" | null>(null);
  ...
  const enabledSides = sides.filter((side) => side.isEnabled);
  const current =
    enabledSides.find((side) => side.id === currentId) || enabledSides[0];
  const shown = useMemo(
    () =>
  ...
  const refreshMessages = async (id = conversationId) => {
    if (id)
      setMessages(await api<Message[]>(`/api/messages?conversationId=${id}`));
  };
  const load = async () => {
    try {
      const session = await api<Account | null>("/api/auth/session");
      if (!session?.user) return setAuthRequired(true);
      setAccount(session);
      const loadedSides = await api<Side[]>("/api/sides");
      const loadedConversations =
        await api<Conversation[]>("/api/conversations");
      setSides(loadedSides);
      setCurrentId(
        (id) => id || loadedSides.find((side) => side.isEnabled)?.id || null,
      );
      setConversations(loadedConversations);
      setConversationId((id) => id || loadedConversations[0]?.id || null);
  ...
  useEffect(() => {
    ...
    const syncViewportHeight = () => {
      shellRef.current?.style.setProperty("--vvh", `${viewport.height}px`);
  ...
  async function create() {
    try {
      ...
      say("Couldn’t create conversation.");
    }
  }
  async function toggleSide(side: Side) {
    try {
      const updated = await api<Side>(`/api/sides/${side.id}`, {
        method: "PATCH",
        body: JSON.stringify({ isEnabled: !side.isEnabled }),
      });
      const next = sides.map((item) => (item.id === side.id ? updated : item));
      setSides(next);
      if (side.id === currentId && !updated.isEnabled)
        setCurrentId(next.find((item) => item.isEnabled)?.id || null);
    } catch (error) {
      say(
        error instanceof Error ? error.message : "Couldn’t update this side.",
      );
    }
  }
  async function send(event: FormEvent) {
    ...
    try {
      ...
      say(error instanceof Error ? error.message : "Couldn’t save message.");
    }
  }
  async function rename(item: Conversation) {
    const title = editingTitle.trim();
    if (!title) return setEditingConversationId(null);
    try {
      const updated = await api<Conversation>(`/api/conversations/${item.id}`, {
        method: "PATCH",
        body: JSON.stringify({ title }),
      });
      setConversations((all) =>
        all.map((entry) => (entry.id === item.id ? updated : entry)),
      );
      setEditingConversationId(null);
    } catch {
      say("Couldn’t rename conversation.");
    }
  }
  async function archive(item: Conversation) {
    try {
      await api(`/api/conversations/${item.id}/archive`, { method: "POST" });
      const remaining = conversations.filter((entry) => entry.id !== item.id);
      setConversations(remaining);
      if (conversationId === item.id)
        setConversationId(remaining[0]?.id || null);
    } catch {
      say("Couldn’t archive conversation.");
    }
  }
  async function removeConversation(item: Conversation) {
    ...
    try {
      ...
      say("Couldn’t delete conversation.");
    }
  }
  async function reorder(sourceId: string, destinationId: string) {
    if (sourceId === destinationId) return;
    const source = conversations.findIndex((item) => item.id === sourceId);
    const destination = conversations.findIndex(
    ...
    try {
      ...
      say("Couldn’t reorder conversations.");
    }
  }
  ...
  async function react(messageId: string, reaction: string) {
    ...
    try {
      await api(`/api/messages/${messageId}/reactions`, {
        method: "POST",
        body: JSON.stringify({ sideId: current.id, reaction }),
      });
      await refreshMessages();
    } catch {
      say("Couldn’t save reaction.");
    }
    setEmojiOpen(null);
    setReactionMessageId(null);
  }
  function pickEmoji(emoji: EmojiClickData) {
    if (emojiOpen === "reaction" && reactionMessageId)
      void react(reactionMessageId, emoji.emoji);
    else {
  ...
  const theme = {
    "--accent": current?.color || "#8179c4",
  } as React.CSSProperties;
  return (
    <main ref={shellRef} className="shell" style={theme}>
      ...
      <div className={`app-layout mobile-${mobileView}`}>
        <aside className="library">
          ...
          {conversations.map((item) => (
            <div
              className={`thread-row ${item.id === conversationId ? "selected" : ""} ${item.id === draggedConversationId ? "dragging" : ""}`}
              key={item.id}
              draggable={editingConversationId !== item.id}
              onDragStart={(event) => {
        ...
        <section className="chat">
          <header className="chat-header">
            ...
            <div className="persona">
              <b>
                ...
                {current?.name || "No side available"}
              </b>
              <small>current side</small>
            </div>
            <button
              className="mobile-side-manager"
          ...
          <div className="messages" ref={messagesRef}>
            {shown.map((message) => {
              ...
              const expanded = activeMessageId === message.id;
              const author = sides.find(
          ...
          <form className="composer" onSubmit={send}>
            ...
            <input
              ...
              onFocus={() => messagesRef.current?.scrollTo({ top: messagesRef.current.scrollHeight })}
            ...
            <span className="emoji-picker">
              <button
                ...
                onMouseDown={(event) => event.preventDefault()}
                onClick={() =>
                  setEmojiOpen(emojiOpen === "draft" ? null : "draft")
                }
              >
                <Smile size={20} />
              </button>
            ...
            <button
              ...
              onMouseDown={(event) => event.preventDefault()}
              onClick={() => {
                const index = enabledSides.findIndex(
                  (side) => side.id === current?.id,
                );
                setCurrentId(
                  enabledSides[(index + 1) % enabledSides.length]?.id || null,
                );
              }}
              aria-label="Switch to next available side"
            >
      ...
      <button className="side-manager" onClick={() => setSidesOpen(true)}>
        Sides · {enabledSides.length} available
      </button>
      ...
      {sidesOpen && (
        <div className="modal-backdrop" onClick={() => setSidesOpen(false)}>
          ...
          >
            ...
            <h2>Your sides</h2>
            <p>
              Tap a color to make it available or unavailable. Your messages
              always stay here.
            </p>
            <div className="sides-grid">
              {sides.map((side) => (
                <button
                  key={side.id}
                  className={`side-tile ${side.isEnabled ? "enabled" : "disabled"} ${side.id === current?.id ? "current" : ""}`}
                  style={{ "--side-color": side.color } as React.CSSProperties}
                  aria-pressed={Boolean(side.isEnabled)}
                  onClick={() => void toggleSide(side)}
                >
                  {side.name}
                </button>
              ))}
