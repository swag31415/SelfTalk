...
export default function SettingsPage() {
  ...
  return <main className="shell settings-shell"><header className="appbar"><div className="brand">Self<span>Talk</span><small>settings</small></div><button className="quiet-button settings-back" aria-label="Back to journal" onClick={() => router.push("/")}><ArrowLeft size={18} /><span>Back to journal</span></button></header><section className="settings-card"><h1>Settings</h1><p>Keep your private journal simple and yours.</p><div className="settings-account">{user?.image ? <img src={user.image} alt="Signed-in account" /> : <span>{(user?.name || user?.email || "You").slice(0, 1).toUpperCase()}</span>}<div><b>{user?.name || "Your Google account"}</b><small>{user?.email || "Loading account…"}</small></div></div><section className="data-controls"><h2>Your data</h2><p>Download a backup, add chats from a SelfTalk export, or permanently clear this journal.</p><button className="settings-action" disabled={busy !== null} onClick={() => void exportData()}><ArrowDownToLine size={18} /> {busy === "export" ? "Exporting…" : "Export all data"}</button><input ref={importInput} className="hidden" type="file" accept="application/json,.json" onChange={(event) => void importData(event)} /><button className="settings-action" disabled={busy !== null} onClick={() => importInput.current?.click()}><FileUp size={18} /> {busy === "import" ? "Importing…" : "Import chats"}</button><button className="settings-action danger" disabled={busy !== null} onClick={() => void deleteAllData()}><Trash2 size={18} /> {busy === "delete" ? "Deleting…" : "Delete all data"}</button></section><button className="new-thread" onClick={() => void signOut({ callbackUrl: window.location.origin })}><LogOut size={18} /> Sign out</button>{notice && <p className="toast">{notice}</p>}</section></main>;
}
